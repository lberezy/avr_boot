...
void fram_init() {
  ...
  spi_send(FRAM_STATUS_WEL); // enable writes, disable other protection
  FRAM_CS_DEACTIVATE();
...
uint8_t fram_read_byte(uint16_t addr) {
  ...
  spi_send(FRAM_MSB_MASK & (uint8_t)(addr >> 8)); // send MSB of address
  spi_send((uint8_t)addr); // send LSB of address
...
void fram_write_byte(uint16_t addr, uint8_t c) {
  ...
  FRAM_CS_DEACTIVATE(); // ensure falling edge
  FRAM_CS_ACTIVATE();
  spi_send(FRAM_WRITE_MEMORY); // issue read opcode
  spi_send(FRAM_MSB_MASK & (uint8_t)(addr >> 8)); // send MSB of address
  spi_send((uint8_t)addr); // send LSB of address
  spi_send(c); // issue 8 clocks to recieve data on
  FRAM_CS_DEACTIVATE();
}
